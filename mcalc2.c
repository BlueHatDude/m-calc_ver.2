...
void DEBUGGING__logTokens(const struct MC2_EquationToken tokens[]) {
  for(unsigned i = 0; i < 25; ++i) {
    switch(tokens[i].type) {
      ...
      case INTEGER:
        ...
        break;
      case DECIMAL:
        printf("Index: %d | Type: DECIMAL | fvalue: %lf \n", i, tokens[i].fvalue);
        break;
    }
  }
}
...
double DEBUGGING_run(const char* equation) {
  double total = 0.0;
  ...
  if( checkEqu(equation) ) {
    ...
    // MLOG_normal("Tokens after read_equ()");
    DEBUGGING__logTokens(tokens);
